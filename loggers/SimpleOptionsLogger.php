...
class SimpleOptionsLogger extends SimpleLogger
{
	...
	function on_updated_option($option, $old_value, $new_value) {
		...
		// We only want to log options being added via /wp-admin/options.php
		if ( basename( $_SERVER["REQUEST_URI"] ) !== "options.php" || basename( dirname( $_SERVER["REQUEST_URI"] ) ) !== "wp-admin" ) {
			return;
		}
