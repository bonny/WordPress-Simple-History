...
-   Composer's resolver is pinned to PHP 7.4 via `composer.json` `config.platform.php`, so never silence a platform error with `--ignore-platform-req=php`. See [docs/composer.md](docs/composer.md).
