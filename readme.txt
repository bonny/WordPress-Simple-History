...
-   Three new columns for `wp simple-history list`, all opt-in via `--fields=`: `date_relative` shows "5 minutes ago" style timestamps for easier scanning; `site` shows the blog name and host (useful when piping or comparing output across installs); `ai_agent` shows the detected AI tool name (Claude Code, ChatGPT, etc.) when an event was initiated through an AI agent.
